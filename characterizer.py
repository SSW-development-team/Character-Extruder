...
def extruder(token: list[str], tag: str):
...
def character_extruder(filename: str):
    """filenameを解析し、元ファイルを新方式に書き換え、キャラクターファイルを出力します"""
    os.chdir(os.getcwd())

    mode = "replace"
    tag = countryTagIdentifier(filename)
    if tag == None:
    ...
    for line in raw_txt.splitlines():
        ...
        if nest_count > 0:
            ...
            characters_local = extruder(token, tag)
